...
router.post('/create', upload.fields([{ name: 'picture', maxCount: 1 }, { name: 'audio', maxCount: 1 }]), [
  ...
  const { title, description, startDate, startTime, deadlineDate, endTime, assignTo,assignedBy } = req.body;
  let picturePath;
  let audioPath;
  ...
  try {
    //const assignedBy = req.user.employeeId; // Ensure this is correctly set
...
async function sendNotification(description, status, sdate,stime,ddate,etime,assBy,phoneNumber) {
    const url = 'https://sms.visionhlt.com/api/mt/SendSMS';
    //const message = `Dear {#var#}, your billing amount is {#var#}. Status: {#var#}.`;
...
router.get('/send-notification', async (req, res) => {
  const { description, status, sdate, stime, ddate, etime, assBy, phoneNumber } = req.query;

  if (!description || !status || !sdate || !stime || !ddate || !etime || !assBy || !phoneNumber) {
      return res.status(400).send('Missing required query parameters');
  }

  try {
      const result = await sendNotification(description, status, sdate, stime, ddate, etime, assBy, phoneNumber);
      res.json(result);
  } catch (error) {
...
// Define the route to fetch task data
router.get('/tasks', async (req, res) => {
  try {
    const tasks = await Task.find({}, 'status title assignTo startTime endTime startDate deadlineDate')
                            .populate('assignTo', 'name') // Assuming you want to populate assignTo with Employee names
                            .populate('assignedBy', 'name'); // Assuming you want to populate assignedBy with Employee names
    res.json(tasks);
  } catch (error) {
